...
(defun ink-process (&optional insert)
  "Use inkspace to create an image and tex. When INSERT is
non-nil, insert at point the tex string associated with the
file."
  (let* ((tdir (expand-file-name ink-temp-dir default-directory))
    ...
    (dolist (file files-all (ink-post-process tdir))
      ...
      (if insert
          (ink-insert-tex file)
        (message "")
        (message (format "Created file %s" file))))))

(defun ink-create-sentinel (process event)
  "Wait for inkscape PROCESS to close but has no use for EVENT."
  (when (memq (process-status process) '(exit signal))
    (ink-process t)))

(defun ink-edit-sentinel (process event)
  "Wait for inkscape PROCESS to close but has no use for EVENT."
  (when (memq (process-status process) '(exit signal))
    (ink-process)))

(defun ink-edit-svg (fsvg &optional arg)
...
(defun ink-make-figure (fig)
  ...
  (let* ((log-buffer (get-buffer-create "*inky-log*"))
    ...
    (make-process :name "inksape"
                  ...
                  :command (list "inkscape" file)
                  :stderr log-buffer
                  :sentinel 'ink-create-sentinel)))

(defun ink-edit-figure (&optional arg)
  "Edit existing figure or related tex file. With a prefix
argument, insert it at point."
  (interactive "P")
  (let* ((fdir (expand-file-name ink-fig-dir default-directory))
